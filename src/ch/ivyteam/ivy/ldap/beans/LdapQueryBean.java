 ...
 * @version bb 22.11.2005 bb: allow ivy attributes ("in.xx") to set url, name, password  
		...
		try {
			...
			if (!resultEnum.hasMoreElements()) {
				...
				if (ivyGridAttribute == null) {
					 ...
					 * argument.set(attribute, null); } if (includeName) {
					 * argument.set(ivyGridNameAttribute, null); }
					 */
				} else {
				...
				while (resultEnum.hasMoreElements() && !onlyOneDone) {
					...
					if (searchResult != null) {
						...
						attrEnum = resultAttributesHashtable.keys();
...
//										argument.set((String) resultAttributesHashtable.get(attribute), jndiAttribute.get());
										setVariable(resultAttributesHashtable.get(attribute).toString(), jndiAttribute.get(), argument, cont);
										onlyOneDone = true;
									}
...
//										argument.set((String) resultAttributesHashtable.get(attribute), jndiAttribute.getAll());
										setVariable(resultAttributesHashtable.get(attribute).toString(), jndiAttribute.getAll(), argument, cont);
									}
								}
							}
						}
				...
				for (int i = 0; i < result.size(); i++) {
					Vector tmpRow = (Vector) result.get(i);
	...
	public void setVariable(String name, Object value, CompositeObject argument, IIvyScriptContext cont) throws NoSuchFieldException, IvyScriptException, PersistencyException {
		try {	
			...
			ivyError("Ein Attribut ist entweder nicht vorhanden oder nicht initialisiert. Bitte dieses erstellen bzw. initialisieren. Das betroffene Attribut heisst: " + name + ".\n\nFreundliche Grüsse\nIhre LdapQueryBean", cont);
		}
	}
