...
public class RulesModel
{
  ...
  private void checkRow(Row row)
  {
    ...
    {
      throw new IllegalArgumentException("Parameter row must have "+expectedCells+". cells but has "+row.getCells().size()+".");
    }
    ...
    {
      throw new IllegalArgumentException("Parameter row must have "+conditionColumns.size()+". condition cells but has "+conditionCells+".");
    }

    long actionCells = row.getCells().stream().filter(cell -> cell instanceof ValueCell).count();
    if (actionCells != actionColumns.size())
    {
      throw new IllegalArgumentException("Parameter row must have "+actionColumns.size()+". value cells but has "+actionCells+".");
    }
    ...
    for (Cell cell : row.getCells())
    {
      ...
      {
        throw new IllegalArgumentException("Parameter row must not have a condition cell after a value cell.");
      }
    }
  }
