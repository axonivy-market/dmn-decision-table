...
public class LdapBinaryAttributeModifierBean extends AbstractUserProcessExtension
{
  ...
  public static class Editor extends AbstractProcessExtensionConfigurationEditor
  {
    ...
    public void setConfiguration(String configString)
    {
      ...
      try
      {
        ...
        pos = 0;
        model = new DefaultTableModel(removeAttributeTable.getModel().getRowCount(), removeAttributeTable
                .getModel().getColumnCount());
        cols = new Vector<>();
        ...
        pos = 0;
        model = new DefaultTableModel(replaceAttributeTable.getModel().getRowCount(), replaceAttributeTable
                .getModel().getColumnCount());
        cols = new Vector<>();
   ...
   * Constructor
   */
  public LdapBinaryAttributeModifierBean()
  {
          ...
          IIvyScriptContext cont) throws Exception
  {
    ...
    while (enumeration.hasMoreElements())
    {
      ...
      // Ivy.log().debug("new Attribute:"+newAttribute);
      value = null;
      ...
      // ADD
      {
        valueStr = attributes.get(attribute).toString();
        ch.ivyteam.ivy.scripting.objects.Binary bin = (ch.ivyteam.ivy.scripting.objects.Binary) getVariable(
                valueStr, cont);
        value = bin.toByteArray();
        if (value != null)
        {
          jndiAttribute = new BasicAttribute(newAttribute);
