	...
	public CompositeObject perform(IRequestId reqID, CompositeObject argument,
		...
		while (enumeration.hasMoreElements()) {
			...
			else  // ADD
			{
				...
				ch.ivyteam.ivy.scripting.objects.Binary  bin = (ch.ivyteam.ivy.scripting.objects.Binary ) getVariable(valueStr, cont);
				value = bin.toByteArray();
				//Ivy.log().debug("valueStr:"+valueStr);
				//Ivy.log().debug("value length:"+bin.length());
				if (value != null) {
					jndiAttribute = new BasicAttribute(newAttribute);
