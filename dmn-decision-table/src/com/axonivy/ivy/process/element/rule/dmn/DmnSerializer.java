...
public class DmnSerializer {
  ...
  private TLiteralExpression createOutputEntry(Cell cell, ColumnType type) {
    ...
    if (cell instanceof ValueCell) {
      ...
      throw new IllegalArgumentException(
              "cell of type " + cell.getClass().getSimpleName() + " is not supported as output");
    }
    return outputEntry;
  }
  ...
  private InputStream toInputStream(TDefinitions definitions) {
    try {
      ...
      jaxbMarshaller.setProperty("com.sun.xml.bind.marshaller.CharacterEscapeHandler",
              new InsecureCharacterEscapeHandler());

      JAXBElement<TDefinitions> rootElement = new JAXBElement<>(new QName(DMN_XML_NAMESPACE, "definitions"),
              TDefinitions.class, definitions);
